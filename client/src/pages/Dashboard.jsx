...
import TrustedActionRegister from '../components/TrustedActionRegister';
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {user?.role === 'viewer' && (
        <section className="space-y-4 rounded-md border border-slate-200 bg-white p-4 shadow-gov">
          ...
          <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            ...
            <button onClick={resetViewerFilters} className="focus-ring w-full rounded-md border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700">
              Reset Filters
            </button>
            <button onClick={exportViewerCsv} className="focus-ring inline-flex w-full items-center justify-center gap-2 rounded-md border border-gold px-4 py-2 text-sm font-bold text-navy">
              <Download className="h-4 w-4" />
              Export CSV
            </button>
            <button onClick={exportViewerPdf} className="focus-ring inline-flex w-full items-center justify-center gap-2 rounded-md bg-navy px-4 py-2 text-sm font-bold text-white">
              <Download className="h-4 w-4" />
              Export PDF
            </button>
          </div>
      ...
      {user?.role === 'reviewer' && (
        <section>
          <div className="mb-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
            ...
            <Link to="/review-queue" className="rounded-md border border-gold px-3 py-1.5 text-center text-xs font-bold text-navy">Open Full Queue</Link>
          </div>
          <SortableTable
            rows={reviewQueue}
      ...
      <TrustedActionRegister rows={trustedRows} />
      ...
      <div className="mt-3 space-y-2">
        {rows.length ? (
          rows.slice(0, 4).map((item) => (
            <p key={item._id} className="break-words rounded bg-white px-2 py-1.5 text-xs font-semibold text-slate-700">
              {formatter(item)}
            </p>
          ))
        ) : (
